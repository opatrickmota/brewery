...
class FavoriteViewModelTest {
    ...
    @RelaxedMockK
    private lateinit var observer: Observer<ViewData<BreweryModel>>
    private lateinit var capturesArguments: MutableList<ViewData<BreweryModel>>

    @RelaxedMockK
    private lateinit var observerList: Observer<ViewData<List<BreweryModel>>>
    private lateinit var capturesArgumentsList: MutableList<ViewData<List<BreweryModel>>>
    ...
    fun `check result and viewdata status when query returns breweries`() = runTest {
        ...
        assert(capturesArgumentsList[0].status == ViewData.Status.LOADING)
        assert(capturesArgumentsList[1].status == ViewData.Status.COMPLETE)
